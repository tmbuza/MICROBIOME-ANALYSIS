...
read_csv("data/metadata/PRJNA477349_metadata.csv", show_col_types = FALSE) %>%  
  rename_all(tolower) %>% 
  rename(bases = run_total_bases) %>% 
  ...
  select(run_accession, ecosystem, isolate, latitude, longitude, mb_bases=bases, description) %>%
...
read_csv("data/metadata/PRJNA685168_metadata.csv", show_col_types = FALSE) %>%  
  rename_all(tolower) %>% 
  rename(bases = run_total_bases) %>% 
  select_if(~sum((!is.na(.))) > 0) %>% # Remove empty columns
  ...
  mutate(bases = round(bases/1E6, digits = 0)) %>% 
  select(run_accession, age, sex, bmi, smoking, steriods, antibiotics, biologic, priortnf, wk14, wk52, mb_bases=bases) %>%
  write_csv("data/metadata/PRJNA685168_tidy_metadata.csv")


read_csv("data/metadata/PRJNA802976_metadata.csv", show_col_types = FALSE) %>%  
  rename_all(tolower) %>%
  rename(bases = run_total_bases) %>% 
  select_if(~sum((!is.na(.))) > 0) %>% # Remove empty columns
  distinct(., run_accession, .keep_all = TRUE) %>% 
  ...
  mutate(bases = round(bases/1E6, digits = 0)) %>% 
  select(run_accession, latitude, longitude, mb_bases=bases)  %>% 
  write_csv("data/metadata/PRJNA802976_tidy_metadata.csv")


library(tidyverse, suppressPackageStartupMessages())

read_csv("data/metadata/PRJEB21612_metadata.csv", show_col_types = FALSE) %>% 
  rename_all(tolower) %>%  
  rename(bases = run_total_bases) %>% 
  distinct(., run_accession, .keep_all = TRUE) %>% 
  mutate(bases = round(bases/1E6, digits = 0)) %>% 
  select(run_accession, latitude = "geographic location (latitude)", longitude = "geographic location (longitude)", mb_bases=bases) %>%
